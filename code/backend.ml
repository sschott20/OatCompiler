...
(* Optimizations :
      1) when spilling, spill the least accessed uid
*)
...
let better_layout (f : Ll.fdecl) (live : liveness) : layout =
  ...
  let fcfg_to_uid_list (cfg : Ll.cfg) : uid list =
    ...
    in
    List.filter (fun str -> str <> "DELETE") (block_to_uidl entry @ block_uids)
  in
  ...
  let get_neighbor_colors (graph : graph) (uid : string) : LocSet.t =
    let neighbors =
      UidSet.elements (UidMap.find uid graph.interference)
      (* failwith (String.concat " " [ uid; "not in interference" ]) *)
    in
    List.fold_left
      (fun acc neighbor ->
        ...
        if UidMap.mem uid g.precolored then color g tl
          ...
          let loc =
            try
              ...
              let available_locs = LocSet.diff pal used_locs in
              let pref_locks =
                try LocSet.inter available_locs (UidMap.find uid g.preference)
                with Not_found -> available_locs
              in
              if LocSet.is_empty pref_locks then
                if LocSet.is_empty used_locs then LocSet.choose available_locs
                else if LocSet.is_empty available_locs then raise Spill
                  (* spill () *)
                else LocSet.choose available_locs
              else LocSet.choose pref_locks
            with Not_found -> raise Spill
            (* spill () *)
          in
  ...
  let final_uid_list = kempe_order g uid_list in

  let preference (g : graph) (cfg : Ll.cfg) : graph =
    let entry, blocks = cfg in

    let block_to_insn_list (block : Ll.block) : Ll.insn list =
      List.map (fun (uid, insn) -> insn) block.insns
    in
    let block_list = entry :: List.map (fun (lbl, block) -> block) blocks in
    let insn_list = List.concat (List.map block_to_insn_list block_list) in
    let rec op_list_pref (opl : Ll.operand list) (g : graph) (arg_num : int) :
        graph =
      match opl with
      | [] -> g
      | hd :: tl -> (
          match hd with
          | Id uid ->
              let pref_loc = arg_loc arg_num in
              let loc_set = LocSet.add pref_loc LocSet.empty in
              let pref = UidMap.add uid loc_set g.preference in

              op_list_pref tl { g with preference = pref } (arg_num + 1)
          | _ -> op_list_pref tl g (arg_num + 1))
    in

    let pref_from_insn (g : graph) (insn : Ll.insn) =
      match insn with
      | Call (ty, op, arg_list) ->
          let op_list = List.map (fun (ty, op) -> op) arg_list in
          op_list_pref op_list g 0
      | _ -> g
    in
    List.fold_left pref_from_insn g insn_list
  in
  ...
  let rec color_args (g : graph) (args : lbl list) (n_arg : int) : graph =
    ...
    | arg :: tl ->
        ...
        in
        let precolored' = UidMap.add arg loc g.precolored in

        color_args { g with precolored = precolored' } tl (n_arg + 1)
  in

  let g = color_args g f.f_param 0 in
  let g = preference g f.f_cfg in

  (* if !Platform.verbose then print_g g else (); *)
  let rec rec_color (g : graph) (uid_list : uid list) (to_precolor : uid list) :
      graph =
    try color g uid_list
    with Spill -> (
      match to_precolor with
      | [] -> g
      | hd :: tl ->
          let precolored' = UidMap.add hd (spill ()) g.precolored in
          rec_color { g with precolored = precolored' } uid_list tl)
  in
  let final_graph = rec_color g final_uid_list final_uid_list in

  (* if !Platform.verbose then print_g final_graph else (); *)
  (* print_g final_graph; *)
  let allocate lo uid =
    (* let colors = color g uid_list in  *)
    let loc =
      try UidMap.find uid final_graph.precolored
      with Not_found -> failwith "should never spill here"
    in
    Platform.verb
    @@ Printf.sprintf "allocated: %s <- %s\n" (Alloc.str_loc loc) uid;
    loc
...
let compile_prog { tdecls; gdecls; fdecls } : X86.prog =
  ...
  let prog = List.map g gdecls @ List.(flatten @@ map f fdecls) in
  (* Printf.printf "num mem op %d\n" (memop_of_prog prog); *)
  prog
